...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@wasit_go"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@SZC_n"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@co_od"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : ali"
    )
...
@ze.on(events.NewMessage(outgoing=True, pattern=".تجميع دعمكم "))
async def _(event):
